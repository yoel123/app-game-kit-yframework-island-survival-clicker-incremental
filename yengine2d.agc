...
global ywasclicked = 0
function is_clicked(e ref as yentity)
	ret = 0
	//GetPointerReleased GetPointerPressed
	if GetPointerState() then ywasclicked = 1

    if GetPointerReleased() and e.yactive and ywasclicked 
        ...
        for TokenID=1 to Tokens
           ...
           settextstring(TextID,TempString$)
            ...
            if gettexttotalwidth(TextID)>=MaxWidth and TokenID>1
                ...
                TempString$ = TempString$+chr(10)+chr(13)+getstringtoken(OriginalString$," ",TokenID)
            endif
        next TokenID
...
function yfiles(n as string)
	
	ret as string
	if GetFileExists( n )
		file = OpenToRead(n)
		while FileEOF(file) = 0
			ret = ret+ Readline(file)+" "
		endwhile
